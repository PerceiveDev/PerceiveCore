...
public class SpigetUpdater extends AbstractUpdater {
    ...
    public UpdateCheckResult searchForUpdate() {
        ...
        for (SpigetResponse spigetResponse : spigetResponses) {
            LocalDateTime releaseDate = LocalDateTime.ofInstant(Instant.ofEpochSecond(spigetResponse.releaseDate), ZoneId.systemDefault());
            URL url;
            try {
    ...
    public UpdateResult update() {
        if (getEntryList().isEmpty()) {
            return UpdateResult.NO_UPDATE_FOUND;
        }
        ...
        try {
            ...
            getPlugin().getLogger().log(Level.WARNING, "Malformed URL, Url was STATICALLY added. This shouldn't happen!", e);
            return UpdateResult.ERROR_WHILE_DOWNLOADING;
        }
        UpdaterEntry fakedSpigetUrl = new UpdaterEntry(realEntry.getName(), realEntry.getVersion(), realEntry.getReleaseTime(), fakedUrl);
