...
public class AxisAlignedLine extends AbstractParticleShape {
    ...
    protected void ensureWoldIsSame(Location one, Location two) {
        ...
        if (!one.getWorld().equals(two.getWorld())) {
            throw new IllegalArgumentException(String.format("Worlds differ! One={%s}, Second={%s}", one.getWorld(), two.getWorld()));
        }
    }
    ...
    private void ensureOneAxis(Location one, Location two) {
        if (Double.compare(one.getX(), two.getX()) != 0
            ...
            throw new IllegalArgumentException(String.format("It covers at least X and Y axes! One={%s}, Second={%s}", one.toVector(), two.toVector()));
        }
        if (Double.compare(one.getX(), two.getX()) != 0
                && Double.compare(one.getZ(), two.getZ()) != 0) {

            throw new IllegalArgumentException(String.format("It covers at least X and Z axes! One={%s}, Second={%s}", one.toVector(), two.toVector()));
        }
        if (Double.compare(one.getY(), two.getY()) != 0
                && Double.compare(one.getZ(), two.getZ()) != 0) {

            throw new IllegalArgumentException(String.format("It covers at least Y and Z axes! One={%s}, Second={%s}", one.toVector(), two.toVector()));
        }
    }
