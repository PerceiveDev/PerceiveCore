...
public class SerializationManager {
    ...
    private Map<Class<?>, SerializationProxy<?>> serializationProxyMap = new HashMap<>();
    ...
    private Map<String, Object> serialize(ConfigSerializable configSerializable, int depth) {
        ...
        for (Field field : getFieldsToSerialize(configSerializable.getClass())) {
            ...
            if (ConfigSerializable.class.isAssignableFrom(type)) {
                ...
                // will actually be the case as I check in the first statement. Just to shut IntelliJ up and don't add any SuppressWarnings
     ...
     *
     * @throws IllegalStateException    if a too deep loop is detected
     * @throws IllegalArgumentException if it doesn't know how to deal with a field
     */
    private <T> T deserialize(Class<T> clazz, Map<String, Object> data, int depth) {
        ...
        for (Field field : getFieldsToSerialize(clazz)) {
            ...
            if (data.containsKey(field.getName())) {
                ...
                // don't let the deserializers deal with nulls. Do it yourself.
                ...
                if (ConfigSerializable.class.isAssignableFrom(type)) {
                    ...
                    if (serializedData instanceof Map) {
                        // is assumed. If this false, the data has been modified.
                    ...
                    // is actually checked by the contains call
                    @SuppressWarnings("unchecked")
                    Map<String, Object> map = (Map<String, Object>) serializedData;
